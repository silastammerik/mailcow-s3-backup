...
This project does not replace Mailcow's backup logic. It runs the official `backup_and_restore.sh` script and then uploads the generated `mailcow-<timestamp>` directory through a configured `rclone` remote.
...
- Credentials and provider-specific settings are handled by the configured `rclone` remote.
...
Then set `RCLONE_REMOTE=mailcow-backups` in `.env`.
